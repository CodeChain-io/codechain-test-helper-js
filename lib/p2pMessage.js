...
class HandshakeMessage {
    ...
    static fromBytes(bytes) {
        ...
        const version = decodedbytes[0].length === 0
            ? new codechain_primitives_1.U256(0)
            : new codechain_primitives_1.U256(parseInt(decodedbytes[0].toString("hes"), 16));
...
class NegotiationMessage {
    ...
    static fromBytes(bytes) {
        ...
        const version = decodedbytes[0].length === 0
            ? new codechain_primitives_1.U256(0)
            : new codechain_primitives_1.U256(parseInt(decodedbytes[0].toString("hex"), 16));
        const protocolId = decodedbytes[1].length === 0 ? 0 : decodedbytes[1].readUIntBE(0, 1);
        const seq = decodedbytes[2].length === 0
            ? new codechain_primitives_1.U256(0)
            : new codechain_primitives_1.U256(parseInt(decodedbytes[2].toString("hex"), 16));
        switch (protocolId) {
...
class ExtensionMessage {
    ...
    static fromBytes(bytes, secret, nonce) {
        ...
        const extensionVersion = decodedbytes[3].length === 0
            ? new codechain_primitives_1.U256(0)
            : new codechain_primitives_1.U256(parseInt(decodedbytes[3].toString("hex"), 16));
